<main
  class="h-[calc(var(--spacing)*-16-1.5em-2px+100vh)] w-screen fixed top-[calc(var(--spacing)*16+1.5em+2px)] z-0"
>
  <div class="flex h-full w-full flex-col items-center justify-center gap-4">
    <h1 class="text-[3rem]/tight">Pink Legal Club</h1>
    <h1 class="text-[10rem]/[10rem]">Evora</h1>
    <button
      class="bg-secondary text-background py-6 px-12 text-2xl rounded-lg shadow-[-6px_6px_0_#af9672]"
      >Our Work</button
    >
  </div>
</main>

<div class="h-screen relative top-[100vh] z-1 bg-red-500"></div>
